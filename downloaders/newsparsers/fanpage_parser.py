...
class FanpageParser:
    ...
    # DO NOT TOUCH THIS
    def __init__(self, target_domain, decode_format = 'UTF-8'):
        ...
        dataset = pd.read_csv('../../dataset/intopic_it_articles.csv', sep = ';', index_col = [0])
    ...
    def get_updated_dataset(self):
        ...
        for index, count in zip(self.dataset.index, range(num_of_articles)):
            ...
            if index % 10 == 0:
                print("Progress at %.2f " % float(count/num_of_articles*100))
...
f = FanpageParser('www.fanpage.it')
f.get_updated_dataset()
